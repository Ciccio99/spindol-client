...
const getTableData = async ({ startDate, endDate, tag1, tag2 }) => {
  if (!startDate || !endDate || !tag1) {
    return undefined;
  }
  const tag1Data = await SleepSummaryServices.getTagSleepTableData(
  ...
  if (tag2) {
    const tag2Data = await SleepSummaryServices.getTagSleepTableData(
      startDate.format('YYYY-MM-DD'),
      endDate.format('YYYY-MM-DD'),
      tag2
    );
...
const TagSleepTable = ({ startDate, endDate, tag1, tag2 }) => {
    ...
    return (
      <>
        ...
        <ComparisonTable
          ...
          stats1CountLabel={`(${data.newStatsCount} data point${
            data.newStatsCount === 1 ? '' : 's'
          })`}
          stats2={data.baselineStats}
          stats2Label={tag2 ? tag2.tag : undefined}
          stats2CountLabel={
            tag2
              ? `(${data.stats2Count} data point${
                  data.stats2Count === 1 ? '' : 's'
                })`
              : undefined
          }
        />
