...
const MoodCard = ({ mood, date }) => {
  ...
  return (
    <Paper elevation={24} className={clsx( {
        [classes.cardGrow]: (width >= 1100 ),
        [classes.cardToday]: sameDay,
      })}>
      <Box px={3} pt={2} pb={1} maxHeight={100} height={100} display="flex" flexDirection="column">
        <Box display="flex" justifyContent="center" mb={1}>
          <MoodIcon size={48} />
        </Box>
        <Typography className={clsx(classes.dayText, { [classes.textToday]: sameDay })} align="center" variant="h4">
