...
import { Box, Typography, Paper, Grid, Popover } from '@material-ui/core';
...
const useStyles = makeStyles((theme) => ({
  totalSleepBg: {
    background: (props) =>
      `linear-gradient(90deg, ${COLORS.LIGHT_GREEN} ${props.percentage}%, ${COLORS.WHITE} ${props.percentage}%)`,
  },
  avgText: {
      ...
      <div>
        <Box mb={2}>
          <Typography variant="subtitle1">
            Your sleep from{' '}
            <span className={clsx(classes.textHighlight)}>last night</span>
          </Typography>
        </Box>
        <SleepTotalCard
  ...
  return (
    <Paper elevation={24} style={{ position: 'relative' }}>
      {data
        ...
        : null}
      <Box
        className={clsx(classes.totalSleepBg)}
        p={2}
        height={200}
        display="flex"
      ...
      >
        ...
        <div>
          ...
          <Typography
            className={clsx(classes.avgText)}
            variant="subtitle1"
            display="inline"
          >{`AVG: ${avg} HRS`}</Typography>
          <Typography
            className={clsx(classes.diffText)}
            variant="subtitle1"
            display={isMobile ? 'block' : 'inline'}
          >{`${delta} HOURS`}</Typography>
        </div>
      </Box>
    </Paper>
  );
...
const SleepMoodLine = ({
  ...
  return (
    <>
      <svg
        className={clsx(classes.sleepMoodLine)}
        width="3"
        height={height || 50}
        viewBox="0 0 3 200"
        fill="none"
      ...
      >
        <line
          className={clsx({
            [classes.sleepMoodLineInverse]:
              parseInt(xPercentage) > parseInt(currPercentage),
          })}
          x1="0"
          y1="0"
          x2="0"
    ...
    <>
      ...
      >
        <Box p={2} maxWidth={280}>
          <Box mb={1}>
            <Typography variant="body1">{`When you get around ${duration.toFixed(
              1
            )} hours of sleep you feel ${mood}`}</Typography>
          </Box>
          <Typography variant="h4">
            {count} {count === 1 ? 'time' : 'times'} slept around{' '}
            <span className={clsx(classes.textHighlight)}>
