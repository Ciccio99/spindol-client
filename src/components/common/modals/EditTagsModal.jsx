...
const getOrderedTags = async () => {
  ...
  const [sleepTags, activityTags] = tags.reduce(
    (tagsArr, tag) => {
      if (tag.sleepTrial) {
        tagsArr[0].push(tag);
      } else {
...
const EditTagsModal = ({ open, currentTags, handleModal, handleSaveTags }) => {
  const { data, isPending, error, setData } = useAsync(getOrderedTags);
  ...
  const handleTagDeSelect = (removeTag) => {
    setSelectedTags((prevState) =>
      prevState.filter((tag) => tag._id !== removeTag._id)
    );
  };
  ...
  const handleTagSubmit = async (tagInput) => {
    ...
    try {
      ...
      setData({
        ...
        activityTags: [...data.activityTags, newTag],
      });
    } catch (e) {
      dispatchAlertSystem({
        type: 'WARNING',
  ...
  const handleCreateSleepTag = React.useCallback(
    async (sleepTrial) => {
      try {
        const dto = {
          tag: sleepTrial.name,
        ...
        setData({
          sleepTags: [...data.sleepTags, newSleepTag],
      ...
      <ModalWrapper open={open} handleOnModalClose={handleOnModalClose}>
        ...
        >
          ...
          >
            ...
            />
            <Box mt={2}>
              <LinkText to={{ pathname: '/settings', hash: 'tags' }}>
                Manage Habit Tags
              </LinkText>
            </Box>
          </Box>
  ...
  return <ModalWrapper open={open} handleOnModalClose={handleOnModalClose} />;
};
...
const EditTagsTab = ({
  ...
  <Box
    pb={10}
    maxHeight={useMobile().isMobile ? '76vh' : '81vh'}
    ...
    >
      <Grid container spacing={2} alignItems="center">
        ...
        <Grid item xs={12} sm={4}>
          <Button
            disabled={!tagInput || disableCreate}
            type="submit"
            color="secondary"
            variant="contained"
            disableElevation
