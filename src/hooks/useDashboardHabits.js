...
const getBedtimeStats = (sleep, habit) => {
  ...
  const sleepBedtime = moment(sleep.startDateTime).utc().utcOffset(sleep.timezoneOffset);
...
const getWaketimeStats = (sleep, habit) => {
  ...
  const sleepWaketime = moment(sleep.endDateTime).utc().utcOffset(sleep.timezoneOffset);
  const targetDateTime = moment(sleep.date, 'YYYY-MM-DD')
    .add(habit.targetValue, 'minutes')
    .utc()
    .utcOffset(sleep.timezoneOffset);
...
const useDashboardHabits = () => useQuery(['dashboard', 'habits'], async () => {
  ...
  if (sleepSummaries && habits) {
    ...
    while (date.diff(end, 'days') <= 0) {
      const sleep = sleepSummaries.find((ss) => moment.utc(ss.date).format('YYYY-MM-DD') === date.format('YYYY-MM-DD'));
      if (sleep) {
        const sleepDate = moment.utc(sleep.date);
        const bedtimeHabit = habits.bedtime.find(
          (habit) => (sleepDate.isBetween(habit.startDate, habit.endDate || moment(), undefined, '[]')),
        );

        const waketimeHabit = habits.waketime.find(
          (habit) => (sleepDate.isBetween(habit.startDate, habit.endDate || moment(), undefined, '[]')),
        );
